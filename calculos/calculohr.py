...
def hrtransporte():
    ...
    try:
        ...
        FROM Cte
        WHERE cnpj_transp = '28493959000125'
        """
        ...
        if rows:
            for row in rows:
                ...
                # Data limite como objeto datetime.date
                data_limite = datetime(2024, 10, 1).date()
                
                if emissao > data_limite:
                    frete =round (valor *0.02,2)
                    if frete < 19.00:
                        frete = 19.00
                else:
                    frete =round (valor *0.02,2)
                ...
                WHERE totalnf > 0 AND cnpj_transp = '28493959000125'
